...
def objective(trial):
    ...
    train_dataloader, dev_dataloader, test_dataloader, class_weights, total_steps, df_dev, df_test = prepare_dataloader(tokenizer)
    ...
    if f1 > F1_THERESHOLD :
        ...
        df_dev.to_csv(f'result/df_dev_trial_{params["trial_num"]}_{round(f1, 4)}.csv')
        df_test['prediction'] = model_prediction(model,test_dataloader)
        df_test.to_csv(f'result/df_test_trial_{params["trial_num"]}_{round(f1, 4)}.csv')

        create_submission_file(df_test, path=f'result/submission_trial_{params["trial_num"]}_{round(f1, 4)}.json')
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(#sampler=TPESampler(seed=42),
                                direction='maximize',  
                                storage="sqlite:///example_2.db", 
                                study_name="stance-clinate-real-tweet",
                                load_if_exists=True)
    study.optimize(objective, n_trials = 3, timeout = 120000)

    df = study.trials_dataframe()
    df.to_csv('optuna_report.csv')
